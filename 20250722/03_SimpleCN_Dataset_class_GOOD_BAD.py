...
class CustomImageDataset(Dataset):
    def __init__(self, root_dir, label_map, transform=None):
        ...
        self.label_map = label_map  # 예: {'BAD': 0, 'GOOD': 1}
...
data_path = "/Users/ngins/Git/python.initial/dataset/carrot"
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(56 * 56 * 32, 128),  # 입력은 CNN에서 전달된 크기, 출력은 보통 64, 128, 256, 512 등
            nn.ReLU(),
...
# 6. 모델 저장
torch.save(model.state_dict(), "carrot_cnn_01.pth")

# 7. 모델 로드 (예시)
model.load_state_dict(torch.load("carrot_cnn_01.pth", map_location=DEVICE))
model.eval()
...
# 9. 예측 실행 예시
predict_image(data_path+'/test/IMG-20250215-WA0000 (1) (1)-9h02Qh.jpg')  # 실제 파일 경로 지정
